...
const Allpages: NextPage = () => {
  return (
    <Layout
      title={`${fiexdSentence.date}H-NEXTで見れるFALENO作品まとめ`}
      description={`H-NEXTで見れるFALENO作品を一覧でまとめています`}
      keyword={fiexdSentence.keywords}
      url={`${fiexdSentence.url}/posts`}
      type="article"
    >
      <h1 className="text-xl sm:text-2xl text-gray-600">{`${fiexdSentence.date}H-NEXTで見れるFALENO作品まとめ【${Datas.length}件】`}</h1>
