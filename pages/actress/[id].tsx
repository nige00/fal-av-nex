...
const Post: NextPage<Props> = ({ Data }) => {
  ...
  return (
    <Layout
      title={`${fiexdSentence.date}で見れる${Data.actressName}のAV【${fiexdSentence.faleno}女優】`}
      description={`H-NEXTで見れる${Data.actressName}の作品まとめ【${fiexdSentence.faleno}女優】`}
      keyword={fiexdSentence.keywords}
      url={`${fiexdSentence.url}/actress/${decodeURI(Data.actressName)}`}
      type="article"
    >
      <h1 className="text-xl sm:text-2xl text-gray-600">{`${fiexdSentence.date}で見れる${Data.actressName}のAV一覧【${fiexdSentence.faleno}女優】【${actressAvList.length}件】`}</h1>
